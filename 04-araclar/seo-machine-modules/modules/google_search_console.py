...
Auth yöntemleri (öncelik sırası):
  1. OAuth2 refresh token (.env → GSC_REFRESH_TOKEN)
  2. Service account JSON (.env → GSC_CREDENTIALS_PATH)
"""
...
class GoogleSearchConsole:
    ...
    # Google Cloud OAuth bilgileri — secrets.env'den okunur
    ...
    def __init__(self, site_url: Optional[str] = None, credentials_path: Optional[str] = None):
        ...
        Args:
            site_url: Site URL (e.g., "https://botfusions.com")
            credentials_path: Service account JSON yolu (opsiyonel)
        """
    ...
    def _build_credentials(self, credentials_path: Optional[str] = None):
        """
        Kimlik doğrulama: önce OAuth2 refresh token, sonra service account dener.
        """
        refresh_token = os.getenv('GSC_REFRESH_TOKEN')

        # Yöntem 1: OAuth2 refresh token
        if refresh_token:
        ...
        raise ValueError(
            "GSC kimlik bilgisi bulunamadı.\n"
            "Seçenek 1: .env dosyasına GSC_REFRESH_TOKEN ekle\n"
            "           → python get_gsc_token.py çalıştır\n"
            "Seçenek 2: .env dosyasına GSC_CREDENTIALS_PATH ekle\n"
            "           → service account JSON yolu"
        )
